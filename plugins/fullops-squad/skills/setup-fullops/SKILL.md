...
의존성이 없으면 `../../DEPENDENCIES.md`의 설치기를 안내한다. 설치기는 Context7을 설치하고 호스트의 MCP 설정으로 연결한다. 도구 목록만으로 미설치를 판정하지 말고 설치 경로와 플러그인 목록도 확인한다. 인증값은 호스트 환경에서 제공하고 레포에 기록하지 않는다.
