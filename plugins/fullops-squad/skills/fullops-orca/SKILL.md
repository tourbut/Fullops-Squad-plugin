...
앱·런타임 도달 여부와 현재 레포 등록 상태를 확인한다. 미정 CLI 배정을 해결하고 기존 워크트리·브랜치를 먼저 조회한다. 필요한 역할의 상설 워크트리만 기본 브랜치에서 만들고 coordinator 체크아웃은 중복 생성하지 않는다. 각 터미널 출력을 읽어 실제 CLI 프롬프트와 작업 경로를 확인한다.
